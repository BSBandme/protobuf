...
def _upload_scenario_result_to_bigquery(dataset_id, table_id, result_file):
    ...
    if not insert_result(bq, dataset_id, table_id, scenario_result):
        print('Error uploading result to bigquery.')
        sys.exit(1)


def insert_result(bq, dataset_id, table_id, scenario_result, flatten=True):
    if flatten:
        _flatten_result_inplace(scenario_result)
    populate_metadata_inplace(scenario_result)
...
def populate_metadata_inplace(scenario_result):
